...
| D-023 | 2026-09-18 | **Second display: local web page.** `heron-base web` serves the same health display and START/STOP controls as the terminal UI on a local web page (`http://127.0.0.1:8765/` by default), as a second thin layer over `LinkClient` (B6). Standard-library HTTP server, one self-contained page (no CDN or web font, so it works offline), browser polls `GET /api/state` and posts `POST /api/command`. No login: it binds `127.0.0.1` by default; `0.0.0.0` exposes control to the network. | Config table `[web]`. `heron-base demo --web` runs it against the fake payload. |
| D-022 | 2026-09-17 | **Metadata for the processing tools:** every flight also gets a `metadata.yml` in the SDR/data-processing team's schema (`band_configurations`, `channel_configurations` with sample-format flags, one `collections` entry per segment file with overflow notes), next to HERON's own `metadata.json`. Each recorder's console log is saved as `<sdr_id>/<prefix>_recorder_log.txt`. Channels name their GNSS band from a `[bands]` config table. | Schema taken from the team's 2026-09-14 B200 captures. `heron-onboard rebuild-metadata` regenerates both files from the segment sidecars after a power loss. |
